...
article_header = ["issn", "e_issn", "publication_name", "volume", "issue", "document_title", "document_type", 
                "issue_date", "publication_date", "published_date", "manuscript_received_date", 
                "manuscript_revised_date", "manuscript_accepted_date", "digital_object_identifier", "abstract", 
                "keywords", "jel_classification"]
article_data = ["1573-6938", "1386-4157", "Experimental Economics"]
...
try:
    # Article Writer
    if path.exists('Articles-Experimental Economics.csv') == False:
        articles_file = open('Articles-Experimental Economics.csv', mode='a+', newline='', encoding='UTF-8')
    ...
    # Author Writer
    if path.exists('Authors-Experimental Economics.csv') == False:
        authors_file = open('Authors-Experimental Economics.csv', mode='a+', newline='', encoding='UTF-8')
...
url = "https://link.springer.com/journal/10683/volumes-and-issues"
driver.get(url)
        ...
        if url_type != 'text/html':
            ...
            try:
                ...
                if len(k_list) == 0:
                    ...
                    keywords_list = re.sub("\n", "", keywords_list)
            except NoSuchElementException:
                keywords_list = "None"

            try:
                jel_class = driver.find_elements_by_xpath("//h3[contains(text(), 'JEL')]/following-sibling::ul[@class='c-article-subject-list'][1]/li/span")
                jel_list = []
                for j in jel_class:
                    jel_list.append(j.text)
                if len(jel_list) == 0:
                    jel_classification = "None"
                else:
                    jel_classification = ', '.join(jel_list)
            except NoSuchElementException:
                jel_classification = "None"

            temp_article_list = [volume, issue, document_title, document_type[i], issue_date, publication_date, published_date, manuscript_received_date, manuscript_revised_date, manuscript_accepted_date, digital_object_identifier, abstract, keywords_list, jel_classification]
