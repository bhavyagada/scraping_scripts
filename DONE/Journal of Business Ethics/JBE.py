...
article_data = ["0167-4544", "1573-0697", "Journal of Business Ethics"]
...
try:
    # Article Writer
    if path.exists('Articles-Journal of Business Ethics.csv') == False:
        articles_file = open('Articles-Journal of Business Ethics.csv', mode='a+', newline='', encoding='UTF-8')
    ...
    # Author Writer
    if path.exists('Authors-Journal of Business Ethics.csv') == False:
        authors_file = open('Authors-Journal of Business Ethics.csv', mode='a+', newline='', encoding='UTF-8')
...
url = "https://link.springer.com/journal/10551/volumes-and-issues"
driver.get(url)
...
for url in issue_urls:
    driver.get(url)
    volume_and_issue = driver.find_element_by_xpath("//div[@class='app-volumes-and-issues__info']/h1")
    vi = re.findall("[\d]+", volume_and_issue.text)
    volume = vi[0]
    issue = vi[1]
    ...
    for url in article_urls:
        ...
        if url_type != 'text/html':
            ...
            try:
                keywords = driver.find_elements_by_xpath("//ul[@class='c-article-subject-list']/li/span")
                k_list = []
                for k in keywords:
            ...
            temp_article_list = [volume, issue, document_title, document_type[i], issue_date, publication_date, published_date, manuscript_received_date, manuscript_accepted_date, digital_object_identifier, abstract, keywords_list]
