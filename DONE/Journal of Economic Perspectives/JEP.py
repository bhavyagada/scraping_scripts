...
article_data = ["0895-3309", "1944-7965", "Journal of Economic Perspectives"]
...
try:
    # Article Writer
    if path.exists('Articles-Journal of Economic Perspectives.csv') == False:
        articles_file = open('Articles-Journal of Economic Perspectives.csv', mode='a+', newline='', encoding='UTF-8')
    ...
    # Author Writer
    if path.exists('Authors-Journal of Economic Perspectives.csv') == False:
        authors_file = open('Authors-Journal of Economic Perspectives.csv', mode='a+', newline='', encoding='UTF-8')
...
for url in issue_urls:
    driver.get(url)
    after_articles = driver.find_elements_by_xpath("//article[contains(text(), 'Articles')]/following-sibling::article/h3/a")
    before_articles = driver.find_elements_by_xpath("//article[contains(text(), 'Feature')]/preceding-sibling::article/h3/a")
    after = []
    before = []
    for a in after_articles:
        a_url = a.get_attribute("href")
        after.append(a_url)
    for b in before_articles:
        b_url = b.get_attribute("href")
        before.append(b_url)

    article_urls = list(set(after).intersection(before))
