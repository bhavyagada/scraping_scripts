...
article_data = ["0198-9073", "1558-8017", "Journal of the American Taxation Association"]
...
try:
    # Article Writer
    if path.exists('Articles-Journal of the American Taxation Association.csv') == False:
        articles_file = open('Articles-Journal of the American Taxation Association.csv', mode='a+', newline='', encoding='UTF-8')
    ...
    # Author Writer
    if path.exists('Authors-Journal of the American Taxation Association.csv') == False:
        authors_file = open('Authors-Journal of the American Taxation Association.csv', mode='a+', newline='', encoding='UTF-8')
...
url = "https://meridian.allenpress.com/jata/issue/browse-by-year"
driver.get(url)
...
for url in all_volumes[2:]:
    ...
    for url in volumes_list:
        ...
        for url in article_urls:
            ...
            try:
                abs = driver.find_elements_by_xpath("//section[@class='abstract']/p")
                if abs and len(abs) < 2:
                    abstract = abs[0].text
                    jel_classification = "None"
                elif abs and len(abs) > 1:
                    abstract = abs[0].text
                    jels = re.search("JEL Classifications:(.*)", abs[-1].text, re.IGNORECASE)
                    if jels != None:
                        jel_classification = jels.group(1).strip()
                    else:
                        jel_classification = "None"
                else:
                    abstract = "None"
            ...
            temp_article_list = [volume, issue, document_title, document_type, issue_date, publication_date, manuscript_received_date, manuscript_accepted_date, digital_object_identifier, abstract, jel_classification]
